...
export class ContractorsRouter {
  ...
  contractorRouter = this.trpc.router({
    ...
    editCondition: this.trpc.authentificatedProcedure
      .input(z.object({
        id: z.string(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const contractorPaymentCondition = await this.contractorsService
          .getContractorPaymentConditionByIdAndOrganizationId(input.id, user.organizationId);

        if (!contractorPaymentCondition) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Not_Found",
          });
        ...
        if (typeof input.completed === "boolean") {
          contractorPaymentCondition.completed = input.completed
        }

        if (typeof input.condition === "string") {
          contractorPaymentCondition.condition = input.condition
        }

        const conditionSaved = await this.contractorsService
          .updateCondition(contractorPaymentCondition);
